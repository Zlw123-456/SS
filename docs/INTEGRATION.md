...
Also verify the gate computation. The current BR uses `Linear -> SiLU -> Linear`, followed by `2.0 * sigmoid(logits)`. Earlier BR code used ReLU and an unscaled sigmoid. These operations introduce no trainable tensors, so strict loading can succeed even when the forward functions differ. Use the gate implementation associated with the selected retrained checkpoint; do not relabel an older checkpoint or infer compatibility from tensor shapes alone.
